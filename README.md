...
<p>We split each dataset into separate training and test sets with a ratio of 90:10. The black-box classifiers were trained on the training set, and the explanations were produced for the outcomes of the black-boxes on the test set.</p>

<h3>BioBERT classifier</h3>
<p>We used the FARM package (<a href="https://github.com/deepset-ai/FARM">https://github.com/deepset-ai/FARM</a>) in python to implement the BioBERT based classification model. As BioBERT is a variation of BERT that was trained on large biomedical text corpora, we used the BERT transformer model with the BioBERT-base language model version 1.0 pretrained on the PubMed and PMC corpora, along with the default hyperparameter settings, unless it is stated. Pretrained BioBERT language models can be downloaded from <a href="https://github.com/dmis-lab/biobert">https://github.com/dmis-lab/biobert</a>.
<br>
...
<p>Although the performance of the black-boxes on the text classification tasks is not among the contributions of our work, we report the classification accuarcy obtained by the black-boxes, in oredr to give an impression of how well the black-boxes performed on the tasks.
</p>
<table>
  <tr>
    <th>
...
- <b>MUSE</b> (https://www-cs.stanford.edu/people/jure/pubs/explanations-aies19.pdf). Based on the method described in the respective paper, we implemented a variation of MUSE that uses words in the itemset mining phase. Consequently, decsion logics of the target black-box are represented by decision rules that assess the presence of specific words in a textual sample.
</p>
<h3>BioCIE</h3>
<p>The following explanations were produced on two samples from the <b>BioText dataset</b>. In these examples, the value of the threshold min_conf was set to 0.7.</p>
...
<p>The following explanations were produced on two samples from the <b>AIMed dataset</b>. In these examples, the value of the threshold min_conf was set to 0.8.</p>
...
<p>The following explanations were produced on two samples from the <b>Hereditary Diseases (HD) dataset</b>. In these examples, the value of the threshold min_conf was set to 0.7. Few first sentences of the sample are represented in this example, since the sample's text is too long and there was no enough space to show the whole text.</p>
<img width="500" src="https://github.com/mmoradi-iut/BioCIE/blob/master/Images/BioCIE-HD.jpg">
<h3>LIME</h3>
<p>The following explanation was produced on a sample from the <b>BioText dataset</b>.</p>
<img width="800" src="https://github.com/mmoradi-iut/BioCIE/blob/master/Images/LIME-BioText.jpg">
<p>The following explanation was produced on a sample from the <b>AIMed dataset</b>.</p>
<img width="800" src="https://github.com/mmoradi-iut/BioCIE/blob/master/Images/LIME-AIMed.jpg">
<p>The following explanation was produced on a sample from the <b>Hereditary Diseases (HD) dataset</b>. Few first sentences of the sample are represented in this example, since the sample's text is too long and there was no enough space to show the whole text.</p>
<img width="800" src="https://github.com/mmoradi-iut/BioCIE/blob/master/Images/LIME-HD(1).jpg">
<h3>MUSE</h3>
<p>The following explanation was produced on a sample from the <b>BioText dataset</b>.</p>
<img width="800" src="https://github.com/mmoradi-iut/BioCIE/blob/master/Images/MUSE-BioText.jpg">
<p>The following explanation was produced on a sample from the <b>AIMed dataset</b>.</p>
<img width="800" src="https://github.com/mmoradi-iut/BioCIE/blob/master/Images/MUSE-AIMed.jpg">
<p>The following explanation was produced on a sample from the <b>Hereditary Diseases (HD) dataset</b>. Few first sentences of the sample are represented in this example, since the sample's text is too long and there was no enough space to show the whole text.</p>
<img width="800" src="https://github.com/mmoradi-iut/BioCIE/blob/master/Images/MUSE-HD.jpg">
